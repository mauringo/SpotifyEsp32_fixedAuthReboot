...
class Spotify {
  ...
  private:
    ...
    const char* _custom_scopes = nullptr;


    /// @brief Secure HTTPS client used only for Spotify Accounts/API traffic.
    WiFiClientSecure _client;

    /// @brief Dedicated OAuth-helper HTTPS client. Kept separate so setInsecure()
    /// never changes the TLS state of the Spotify client.
    WiFiClientSecure _auth_client;
